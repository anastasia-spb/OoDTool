...
class EmbedderPipelineThread(QThread, EmbedderPipeline):
    ...
    # constructor
    def __init__(self, metadata_file: str, data_dir: str, model_wrapper_name, use_cuda: bool,
                 requires_grad: bool, **kwargs):
        super(EmbedderPipelineThread, self).__init__(metadata_file=metadata_file, data_dir=data_dir,
                                                     model_wrapper_name=model_wrapper_name, use_cuda=use_cuda,
                                                     **kwargs)
        self.requires_grad = requires_grad

    def run(self):
        try:
            super(EmbedderPipelineThread, self).predict(self._signal.emit, requires_grad=self.requires_grad)
